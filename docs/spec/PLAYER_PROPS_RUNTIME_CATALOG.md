...
| 31 | `PLPROP_UDPPORT` | `GInt` | Stores UDP port. | If loaded and id valid, sends `PLO_OTHERPLPROPS + id + PLPROP_UDPPORT + raw int` to any client except self. Generic tail also applies. | Parser/applier implemented as state mutation. Live-world forwarding now emits the source-confirmed direct global client broadcast before the generic local forwarding tail. The direct broadcast is modeled only inside the active live-session forwarding boundary; lower-level loaded/id gating remains blocked until production session state models `m_loaded` exactly. |
...
| 53 | `PLPROP_PSTATUSMSG` | `GUChar` | Stores player-list status message. | If loaded and id valid, sends `PLO_OTHERPLPROPS + id + PLPROP_PSTATUSMSG + GCHAR status` to all except self. Generic tail also applies because `__sendLocal[53]` is true. | Parser/applier implemented as state mutation. Live-world forwarding now emits the source-confirmed direct global client broadcast before the generic local forwarding tail. The direct broadcast is modeled only inside the active live-session forwarding boundary; lower-level loaded/id gating remains blocked until production session state models `m_loaded` exactly. |
