...
| 21 | `PLPROP_HORSEGIF` | `GCHAR len` + bytes, max read `219` | Stores horse image; old clients append `.gif` when extensionless. | Generic forwarding. | Parser/applier implemented for modern state mutation. Old-client suffix and generic forwarding remain blocked until version-aware/runtime-state forwarding exists. |
